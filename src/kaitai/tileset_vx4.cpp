...
void tileset_vx4_t::_read() {
    m_array = new std::vector<megatile_type_t*>();
    {
        int i = 0;
        while (!m__io->is_eof()) {
            m_array->push_back(new megatile_type_t(m__io, this, m__root));
            i++;
        }
    }
}
...
void tileset_vx4_t::_clean_up() {
    if (m_array) {
        for (std::vector<megatile_type_t*>::iterator it = m_array->begin(); it != m_array->end(); ++it) {
            delete *it;
        }
